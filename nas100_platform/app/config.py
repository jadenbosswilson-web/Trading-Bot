...
# data source (see data_source.OandaDataSource). This is a single
# server-level token (Jay's own OANDA account), not a per-user
# credential, so it lives here alongside the other server secrets
# rather than in the per-user broker_credential table. Optional: if
# unset, the app falls back to the Yahoo Finance proxy, then to the
# synthetic simulator — see get_data_source_for_user().
