...
def evaluate_ict(
    ...
    if htf_bias is None:
        ...
        confluences.append(_kill_zone_confluence(kz, require_kill_zone))
    ...
    zone = None
    zone_kind = None
    if ob is not None and ob.low <= price <= ob.high:
        zone, zone_kind = (ob.low, ob.high), "order block"
    elif fvgs:
        nearest = min(fvgs, key=lambda g: abs(price - (g.top + g.bottom) / 2))
        if nearest.bottom <= price <= nearest.top:
            zone, zone_kind = (nearest.bottom, nearest.top), "fair value gap"
    if zone is None and ote is not None and ote[0] <= price <= ote[1]:
        zone, zone_kind = ote, "OTE (62-79% retracement)"

    confluences.append(Confluence(
        "Entry zone (OB / FVG / OTE)",
        zone is not None,
        f"Price {price:.2f} is inside a {zone_kind} zone [{zone[0]:.2f}, {zone[1]:.2f}]"
    ...
    # approximated. `active` marks whichever one matches the collapsed
    # `zone`/`zone_kind` result above (there can only be one active zone,
    # since the OB/FVG/OTE check above is a priority chain).
    _KIND_LABEL = {"order_block": "order block", "fvg": "fair value gap", "ote": "OTE (62-79% retracement)"}

    def _zone(ztype: str, direction: Direction, top: float, bottom: float,
              from_time: int, to_time: int | None, label: str) -> dict:
        return {
            "type": ztype, "direction": direction,
            "top": round(top, 2), "bottom": round(bottom, 2),
            "from_time": from_time, "to_time": to_time, "label": label,
            "active": zone_kind is not None and _KIND_LABEL.get(ztype) == zone_kind,
        }

    zones: list[dict] = []
    if ob is not None:
        zones.append(_zone("order_block", ob.direction, ob.high, ob.low, ltf[ob.index].time, None, "Order block"))
    for g in fvgs:
        zones.append(_zone("fvg", g.direction, g.top, g.bottom, ltf[max(0, g.index - 1)].time, None, "Fair value gap"))
    if ote is not None:
        zones.append(_zone("ote", htf_bias, ote[1], ote[0], ltf[leg_start].time, ltf[trigger.index].time, "OTE 62-79% retracement"))
    zones.append({
        ...
        "label": f"{trigger.type} level", "active": False,
    })
    if sweep_ok:
        zones.append({
            "type": "liquidity_sweep", "direction": sweep.direction,
            "top": round(sweep.level, 2), "bottom": round(sweep.level, 2),
    ...
    # bias, structure shift, sweep, zone, kill zone, and no-news are all
    # required for a "sniper" grade signal — this is deliberately strict.
    required = confluences[:6]
    all_required_met = all(c.met for c in required)

    signal: Signal = "FLAT"
    stop_loss = target = None
    if all_required_met:
        signal = "BUY" if htf_bias == "bullish" else "SELL"
        sweep_level = sweep.level if sweep_ok else (ob.low if htf_bias == "bullish" else ob.high) if ob else price
        buffer = abs(price) * 0.0006  # small buffer beyond the sweep/OB
        if signal == "BUY":
            stop_loss = round(min(sweep_level, ob.low if ob else sweep_level) - buffer, 2)
            stop_dist = price - stop_loss
            target = round(price + stop_dist * risk_reward, 2)
        else:
            stop_loss = round(max(sweep_level, ob.high if ob else sweep_level) + buffer, 2)
            stop_dist = stop_loss - price
            target = round(price - stop_dist * risk_reward, 2)

    reason = (
        f"All confluences aligned — {signal} sniper entry" if all_required_met
        else "Missing: " + ", ".join(c.name for c in required if not c.met)
    )
...
def _kill_zone_confluence(kz: str | None, required: bool) -> Confluence:
    if not required:
        return Confluence("Kill zone timing", True, f"Kill zone filter disabled (current: {kz or 'none'})")
