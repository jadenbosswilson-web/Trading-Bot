...
def evaluate_ict(
    ...
    entry_reset_after: int | None = None,
    exclude_zone_keys: frozenset[str] | None = None,
) -> ICTResult:
    """
    exclude_zone_keys: a set of "top.toFixed(2):bottom.toFixed(2)" zone
    identities (see the internal _zone_key helper, matching index.html's
    own zoneKey()) to skip when the entry-zone search would otherwise
    pick one of them — set together with entry_reset_after by the "Reset
    entry target" button, one more key added every time it's clicked, so
    a second reset can't un-exclude the first zone and let a higher-
    priority stale candidate silently come back. This is the important
    half for the common case of a single ongoing trend with only one
    structure trigger so far: entry_reset_after alone can't offer
    anything else in that case (there's no *other* trigger to fall back
    to), but this lets the search move on to a different candidate
    (another FVG further down the trend, etc.) from that same trigger
    instead of going blank until an entirely new BOS/CHoCH eventually
    forms.

    entry_reset_after: epoch-ms timestamp (candle .time units), or None.
    When set, the entry-zone search (breaker/OB/FVG/OTE) ignores every
    structure trigger at or before this time — see the "Reset entry
    target" button on the dashboard. Lets the trader manually move past
    a still-valid but stale "starred" zone the sticky search would
    otherwise keep favoring, in favor of whatever forms next. None (the
    default) preserves the original unrestricted sticky-search behavior.
    ...
    # frontend already builds (see index.html's zoneKey()) for tracking
    # the confirmed/starred zone — reused here so the "Reset entry
    # target" button can say "skip this exact zone" rather than only
    # "skip this whole trigger" (see exclude_zone_keys below).
    def _zone_key(top: float, bottom: float) -> str:
        return f"{round(top, 2):.2f}:{round(bottom, 2):.2f}"

    _excluded_keys: frozenset[str] = exclude_zone_keys or frozenset()
    ...
    def _zone_candidates_for_trigger(trig: StructureEvent) -> dict:
        ...
        # Each tier below additionally skips a candidate whose key is in
        # _excluded_keys (every zone the user has reset past so far) and
        # falls through to the next-priority candidate
        # instead — rather than only being able to move to a whole new
        # *trigger*. Without this, resetting while there's only ever been
        # one structure trigger so far (the common case: no fresh
        # BOS/CHoCH has happened yet) had nothing else to fall back to
        # and just went blank until an entirely new trigger eventually
        # formed, even when this same trigger had another perfectly good
        # candidate (a different FVG further down the trend, etc.)
        # available right now.
        _zone = None
        _zone_kind = None
        _active_brk = False
        ...
        _active_ote = False
        if (
            _brk is not None and _brk_tapped and not _brk_invalidated
            and _zone_key(_brk.high, _brk.low) not in _excluded_keys
        ):
            _zone, _zone_kind = (_brk.low, _brk.high), "breaker block"
            _active_brk = True
        if (
            _zone is None and _ob is not None and _ob_tapped and not _ob_invalidated
            and _zone_key(_ob.high, _ob.low) not in _excluded_keys
        ):
        ...
        if _zone is None and _fvgs:
            _live = [
                g for g in _fvgs
                if _fvg_status[g.index][0] and not _fvg_status[g.index][1]
                and _zone_key(g.top, g.bottom) not in _excluded_keys
            ]
            if _live:
        ...
        if (
            _zone is None and _ote is not None and _ote_tapped and not _ote_invalidated
            and _zone_key(_ote[1], _ote[0]) not in _excluded_keys
        ):
    ...
    # entry_reset_after (set by the frontend's "Reset entry target"
    # button, see routers/trading.py) is the user manually overriding
    # the sticky search above: "stop offering me that old zone, even
    # though it's still technically tapped-and-valid — I want whatever
    # forms next." Any trigger at or before this timestamp is excluded
    # from ever resolving a zone, on both the main search loop and its
    # final fallback below, so the currently-starred zone can't just
    # come right back from the fallback path after being skipped in the
    # loop.
    def _within_reset(trig: StructureEvent) -> bool:
        return entry_reset_after is None or ltf[trig.index].time > entry_reset_after

    resolved = None
    for _trig in reversed(search_triggers):
        if not _within_reset(_trig):
            continue
        ...
        if _r["zone"] is not None:
            ...
            break
    if resolved is None:
        # Falls back to the newest trigger's own (likely still-untapped)
        # candidates for "not tapped yet" display, same as before this
        # reset mechanism existed — still shown so the trader can see
        # what's forming, with the same excluded-zone skip applied so a
        # reset can't just bounce right back to the zone it was meant to
        # move past. But if a trigger-level reset is active and even this
        # newest trigger predates it, the zone/active flags are
        # explicitly cleared so nothing gets silently picked as the
        # entry target; the OB/FVG/breaker candidates themselves stay
        # visible on the chart either way.
        resolved = _zone_candidates_for_trigger(trigger)
        if not _within_reset(trigger):
            resolved = {
                **resolved, "zone": None, "zone_kind": None,
                "active_brk": False, "active_ob": False,
                "active_fvg_index": None, "active_ote": False,
            }
