...
def evaluate_ict(
    ...
    if ob is not None:
        tapped, invalidated = _tap_status(ob.index + 1, ob.high, ob.low, htf_bias)
        if tapped and not invalidated:
    ...
    if zone is None and fvgs:
        live = []
        for g in fvgs:
            tapped, invalidated = _tap_status(g.index + 2, g.top, g.bottom, htf_bias)
            if tapped and not invalidated:
                live.append(g)
        if live:
    ...
    if zone is None and ote is not None:
        tapped, invalidated = _tap_status(trigger.index + 1, ote[1], ote[0], htf_bias)
        if tapped and not invalidated:
    ...
    def _zone(ztype: str, direction: Direction, top: float, bottom: float,
              from_time: int, to_time: int | None, label: str, active: bool = False) -> dict:
        return {
    ...
    zones: list[dict] = []
    if ob is not None:
        zones.append(_zone("order_block", ob.direction, ob.high, ob.low, ltf[ob.index].time, None, "Order block", active_ob))
    for g in fvgs:
        zones.append(_zone("fvg", g.direction, g.top, g.bottom, ltf[max(0, g.index - 1)].time, None, "Fair value gap", g.index == active_fvg_index))
    if ote is not None:
        zones.append(_zone("ote", htf_bias, ote[1], ote[0], ltf[leg_start].time, ltf[trigger.index].time, "OTE 62-79% retracement", active_ote))
    zones.append({
