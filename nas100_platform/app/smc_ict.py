...
def evaluate_ict(
    ...
    def _zone_candidates_for_trigger(trig: StructureEvent) -> dict:
        _sweep = find_liquidity_sweep(ltf, ltf_swing_highs, ltf_swing_lows, trig.index, sweep_lookback)
        ...
        _fvgs = [g for g in find_fvgs(ltf, _leg_start, min(len(ltf) - 1, trig.index + 5), htf_bias) if not g.mitigated]
    ...
    resolved = None
    for _trig in reversed(matching):
    ...
    if ote is not None:
        zones.append(_zone("ote", htf_bias, ote[1], ote[0], ltf[zone_leg_start].time, ltf[zone_trigger.index].time, "OTE 62-79% retracement", active_ote, ote_invalidated))
    zones.append({
