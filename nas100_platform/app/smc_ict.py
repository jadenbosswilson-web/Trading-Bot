...
class ICTResult:
    ...
    # — where liquidity is concentrated across the fetched history, not
    # any single event. Each entry is {top, bottom, intensity} with
    # intensity normalized to [0, 1] against the hottest bucket in this
    # same result. Buckets with zero weight are omitted, so this is
    # typically a small, sparse list rather than one entry per bucket.
...
def build_liquidity_heatmap(
    ...
    across the whole fetched history — not a single line or a single
    event, but a continuous "how hot is this price" read built by
    layering every liquidity-relevant signal this module already
    computes:

    - Every liquidity pool point (see find_liquidity_pools — the loose,
      inclusive-comparison definition, deliberately used here instead of
      the strict fractal one, since "equal highs/lows piling into the
      same price" is exactly the clustering a heat map should surface)
      adds weight to its own bucket.
    - Every confirmed liquidity grab (see find_all_liquidity_grabs) adds
      extra weight — a level that's actually been swept is more
      significant than one that's simply sitting there untouched.
    - Every breaker block (see find_breaker_blocks) adds weight across
      the whole zone it spans, not just a single price point.

    Recency matters: older events are linearly downweighted (oldest bar
    in the window counts at 30% strength, the most recent at 100%), so
    a heat map built from hundreds of bars of history isn't dominated by
    a liquidity pool from days ago that's since become irrelevant.
    ...
    for i in liquidity_highs:
        weights[_bucket_of(candles[i].high)] += 1.0 * _recency(i)
    for i in liquidity_lows:
        weights[_bucket_of(candles[i].low)] += 1.0 * _recency(i)
    for g in grabs:
        weights[_bucket_of(g.level)] += 1.5 * _recency(g.index)
    for b in breakers:
        b_lo, b_hi = _bucket_of(b.low), _bucket_of(b.high)
        for bi in range(min(b_lo, b_hi), max(b_lo, b_hi) + 1):
            weights[bi] += 1.2 * _recency(b.formed_at_index)

    # A tight, quiet consolidation range can pile many pool points into
    # one bucket (real quiet sessions do this; degenerate all-identical
    # test data does it even harder) — left as a raw linear sum, that
    # single crowded bucket would swamp the normalization and wash out
    # every other genuine cluster to near-zero by comparison. sqrt()
...
def evaluate_ict(
    ...
    # Liquidity heat map (see build_liquidity_heatmap) — a continuous
    # price-density read layering the loose liquidity pools, every grab,
    # and every breaker block already computed above. Computed
    ...
    liquidity_heatmap: list[dict] = build_liquidity_heatmap(
        ltf, ltf_liquidity_highs, ltf_liquidity_lows, all_liquidity_grabs, ltf_breakers,
    )
