...
def evaluate_ict(
    ...
    # Liquidity pools (see find_liquidity_pools) and every grab of them
    # (see find_all_liquidity_grabs) are also computed unconditionally —
    # buy-side (above swing highs) and sell-side (below swing lows)
    # liquidity, and every single wick-through-and-reject of either,
    # across the whole candle history, not just whichever one sweep
    # happens to sit right before the current structural trigger (that
    # narrower, trade-relevant search still happens separately below,
    # via find_liquidity_sweep, to gate the actual signal/stop-loss).
    ltf_liquidity_highs, ltf_liquidity_lows = find_liquidity_pools(ltf, swing_lookback)
    all_liquidity_grabs: list[SweepEvent] = find_all_liquidity_grabs(ltf, ltf_liquidity_highs, ltf_liquidity_lows)
    ...
    def _liquidity_grab_zones(_active_index: int | None = None) -> list[dict]:
        ...
        for g in all_liquidity_grabs:
            ...
            out.append({
                ...
                "from_time": ltf[g.index].time, "to_time": None,
    ...
    # ltf_liquidity_highs/lows (see find_liquidity_pools) — the more
    # inclusive pool used for liquidity sweep detection — were already
    # computed unconditionally above, alongside all_liquidity_grabs.
    # Order blocks and the fallback stop level still key off the
    # stricter ltf_swing_highs/lows above, so that pool only widens what
    # counts as sweepable liquidity, without loosening structure/order-
    # block logic.
