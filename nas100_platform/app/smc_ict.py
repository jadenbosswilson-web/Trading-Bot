...
def evaluate_ict(
    ...
    # pick).
    #
    # The tap check must never start before `trigger.index + 1` (the
    # first candle after the confirmed structure shift). An order block
    # is found by scanning *backward* from the trigger, so the candles
    # between the OB and the trigger are the impulsive breakout leg
    # itself — price moving away from the zone to confirm it, not a
    # pullback into it. Starting the tap window at ob.index+1 (or, for
    # an FVG, right as its 3rd candle closes) meant that leg's own
    # candles — which can easily wick back through the zone's edge while
    # the move is still developing — got counted as a "tap", so the
    # checklist reported the zone as tapped almost the instant it
    # formed, before any real retracement had happened. Clamping to
    # trigger.index+1 restricts "tapped" to what it's actually meant to
    # mean: price came back into the zone *after* the move that created
    # it was already confirmed. OTE already anchored to trigger.index
    # this way; OB and FVG now match.
    ob_tapped = ob_invalidated = False
    if ob is not None:
        ob_tap_start = max(ob.index + 1, trigger.index + 1)
        ob_tapped, ob_invalidated = _tap_status(ob_tap_start, ob.high, ob.low, htf_bias)

    fvg_status: dict[int, tuple[bool, bool]] = {
        g.index: _tap_status(max(g.index + 2, trigger.index + 1), g.top, g.bottom, htf_bias) for g in fvgs
    }
