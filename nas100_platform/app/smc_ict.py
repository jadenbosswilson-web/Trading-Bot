...
def evaluate_ict(
    ...
    manual reset mechanism (an earlier entry_reset_after/exclude_zone_keys
    param pair was removed because it only patched around the real bug:
    zone selection used to be priority-based, breaker > OB > FVG > OTE,
    so a stale-but-still-valid higher-priority zone could permanently
    outrank a fresher, more relevant one. Recency-based callouts fix that
    at the root instead of requiring the trader to manually dismiss the
    stale zone).
    ...
    # frontend already builds (see index.html's zoneKey()) — reused here
    # purely as a stable zone *identity* for the callout feed below, so a
    ...
    def _zone_candidates_for_trigger(trig: StructureEvent) -> dict:
        ...
        # No priority selection here anymore — this just returns every
        # raw candidate (breaker/OB/live FVGs/OTE) for this one trigger,
        ...
        # trigger in the recency window at once, by the callout-merging
        # step below — that's what fixes the old "stale higher-priority
        # zone permanently blocks a fresher lower-priority one" bug.
        return {
    ...
    # The callout search must stay bounded to *recent* triggers —
    ...
    # formed afterward from ever becoming the displayed entry zone.
    # Limiting the search to triggers within the last few hours keeps
    # things bounded without letting a setup linger indefinitely once
    # the market has clearly moved on to something new.
    ...
    # Candidates are computed once per trigger and cached — reused both
    # for building the callout feed below and for the chart overlay /
    # stop-loss fallback levels further down.
    _candidates_cache: dict[int, dict] = {}

    def _get_candidates(trig: StructureEvent) -> dict:
        if trig.index not in _candidates_cache:
            _candidates_cache[trig.index] = _zone_candidates_for_trigger(trig)
        return _candidates_cache[trig.index]

    # Build the callout feed: every distinct touch (_tap_events) of every
    # candidate zone (breaker/OB/each live FVG/OTE) across every trigger
    # in the recency window, merged into one list and sorted by when the
    # touch actually happened — not by trigger recency, and not by a
    # fixed priority order. This is the fix for the real bug ("new zones
    # aren't getting rechecked"): a stale breaker touched hours ago no
    # longer permanently blocks a fresh FVG touched a minute ago, because
    # nothing here picks "highest priority tapped zone" — it picks
    # "whatever was touched most recently, period."
    _touch_events: list[dict] = []
    for _trig in search_triggers:
        _cand = _get_candidates(_trig)
        _brk, _ob, _fvgs, _ote = _cand["brk"], _cand["ob"], _cand["fvgs"], _cand["ote"]
        if _brk is not None:
            for _idx in _tap_events(_brk.index + 1, _brk.high, _brk.low, htf_bias):
        ...
        if _ob is not None:
            for _idx in _tap_events(_ob.index + 1, _ob.high, _ob.low, htf_bias):
                _touch_events.append({
        ...
        for _g in _fvgs:
            for _idx in _tap_events(_g.index + 2, _g.top, _g.bottom, htf_bias):
                _touch_events.append({
        ...
        if _ote is not None:
            for _idx in _tap_events(_trig.index + 1, _ote[1], _ote[0], htf_bias):
                _touch_events.append({
    ...
    if callouts:
        # The most recent callout — whichever zone was genuinely touched
        # most recently, from any trigger in the window — automatically
        # becomes the live entry zone. No manual reset needed: as soon
        # as a fresher zone gets tapped, it's next in this list and takes
        # over on the very next poll.
        _last = callouts[-1]
        zone_trigger = next(t for t in search_triggers if t.index == _last["trigger_index"])
        zone = (_last["bottom"], _last["top"])
        zone_kind = _last["zone_kind"]
    else:
        # Nothing has been touched yet anywhere in the window — fall
        # back to the newest trigger's own (still-untapped) candidates
        # purely for "not tapped yet" display; the entry zone itself
        # stays unset.
