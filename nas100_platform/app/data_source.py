"""
Per-user data source resolution. This app has no broker connection —
every user sees the same shared, real market data (OANDA if configured,
else the Yahoo Finance proxy, else a clearly-labeled synthetic
simulator if neither is reachable). "Confirm order" always just logs a
paper trade for the user's own tracking; nothing here ever places a
real order anywhere.
"""
...
class YahooDataSource:
    ...
    expect a small, normal basis difference). Order placement is
    paper-only: there's no account behind this data source to place a
    real order against, so "confirm" just logs what you would have done
    for your own tracking while you execute manually on your own
    broker/platform.
    ...
    def place_order(self, **kwargs) -> dict:
        return {
            ...
            "status": "PAPER_LOGGED",
            "note": "Recorded for your own tracking only. Execute manually on your own broker/platform if you want a real fill.",
            **kwargs,
        }
...
class OandaDataSource:
    ...
    def place_order(self, **kwargs) -> dict:
        return {
            "orderCode": kwargs.get("client_order_id") or "paper-trade",
            "status": "PAPER_LOGGED",
            "note": "Recorded for your own tracking only. Price data is real-time from OANDA, but orders are not sent anywhere; execute manually on your own broker/platform if you want a real fill.",
            **kwargs,
        }
