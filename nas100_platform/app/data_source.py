...
class OandaDataSource:
    ...
    only order execution and money are fake, and this class never
    places OANDA orders at all (see place_order() below), so it's used
    purely as a price feed.
    ...
    def place_order(self, **kwargs) -> dict:
        return {
            "orderCode": kwargs.get("client_order_id") or "paper-trade",
            "status": "PAPER_LOGGED",
            "note": "No Liquid Charts broker connected — recorded for your own tracking only. Price data is real-time from OANDA, but orders are not sent anywhere; execute manually on your actual broker if you want a real fill.",
            **kwargs,
        }
...
class SimulatedExecutionDataSource:
    ...
    dashboard either — every call here falls back to the best other
    real data source available (OANDA if configured, else the Yahoo
    proxy), and `used_fallback` / `fallback_label` record that so the
    frontend can label the data accurately instead of silently
    mislabeling it as broker-sourced."""
    ...
    def _get_fallback(self):
        if self._fallback is None:
            self._fallback = get_shared_fallback_source()
    ...
    def get_candles(self, symbol: str, candle_type: str = "5m", count: int = 200) -> list[dict]:
        try:
            ...
            return self._get_fallback().get_candles(symbol, candle_type, count=count)
    ...
    def get_quote(self, symbol: str) -> dict:
        try:
            ...
            self.used_fallback = True
            return self._get_fallback().get_quote(symbol)
    ...
    def place_order(self, **kwargs) -> dict:
        return {
            "orderCode": kwargs.get("client_order_id") or "dry-run-order",
            "status": "SIMULATED_FILLED",
            "note": "Dry-run mode — no real order was sent (price data is live from your broker account, or from a fallback real-data source if your broker connection is still unverified).",
            **kwargs,
        }
...
_oanda_unreachable = False  # sticky per-process: don't hammer OANDA with a probe on every request once it's confirmed down/misconfigured
...
def get_shared_fallback_source():
    """Best real (non-synthetic) data source available for any
    fallback path — OANDA first (genuinely real-time), Yahoo second
    (real but delayed), used both when a user has no broker connected
    at all and when a connected-but-broken broker needs somewhere to
    fall back to."""
    global _oanda_unreachable
    oanda = get_shared_oanda_source()
    if oanda is not None and not _oanda_unreachable:
        try:
            oanda.get_candles(config.OANDA_INSTRUMENT, "5m", count=5)
            return oanda
        except Exception as e:
            logger.warning("OANDA unreachable, falling back to Yahoo: %s", e)
            _oanda_unreachable = True
    return get_shared_yahoo_source()
...
def is_simulated_source(data_source) -> bool:
    ...
    reachable) — false for OandaDataSource/YahooDataSource (real
    market data) and false for SimulatedExecutionDataSource (real
    broker data, simulated order placement only). Used to show a clear
    "this data isn't real" banner on the dashboard instead of leaving
    it ambiguous why the chart looks off."""
    return isinstance(data_source, DryRunDataSource)


def data_source_label(data_source) -> str:
    """One of 'simulated' | 'yahoo_proxy' | 'oanda' | 'broker' — lets
    the frontend show the right explanation for why the data looks the
    way it does, rather than a single real/fake boolean that can't
    distinguish "fake data" from "real data, just not from your
    broker". A SimulatedExecutionDataSource that had to fall back
    (broker credentials saved but not actually working) reports
    whichever fallback source actually served the data, not 'broker'."""
    ...
    if isinstance(data_source, SimulatedExecutionDataSource) and data_source.used_fallback:
        fb = data_source._fallback
        if isinstance(fb, OandaDataSource):
            return "oanda"
        return "yahoo_proxy"
    return "broker"
...
def get_data_source_for_user(user: User):
    ...
    if user.broker_credential is None:
        # No broker credentials saved — default to the best real,
        # free market data available (OANDA's genuinely real-time feed
        # if configured, else Yahoo's delayed NQ futures feed) instead
        # of a fake random walk, since that's strictly more useful and
        # doesn't require handing over broker credentials at all.
        try:
