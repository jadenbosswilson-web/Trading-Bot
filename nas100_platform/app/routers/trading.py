...
def get_signal(
    ...
    # Comma-separated list of zone keys from the "Reset entry target"
    # button (see smc_ict.py's evaluate_ict docstring) -- one more key
    # gets appended by the frontend every time it's clicked, so this can
    # legitimately be several entries long after repeated resets.
    exclude_zone_keys: str | None = None,
    ...
    result = smc_ict.evaluate_ict(
        ...
        # Set by the dashboard's "Reset entry target" button (see
        # smc_ict.py's evaluate_ict docstring) -- lets the trader
        # manually move past a still-valid but stale "starred" zone the
        # sticky search would otherwise keep favoring. exclude_zone_keys
        # is the important half when there's only been one structure
        # trigger so far (the common case): it can fall through to a
        # different candidate from that SAME trigger instead of only
        # being able to wait for an entirely new one.
        entry_reset_after=entry_reset_after,
        exclude_zone_keys=frozenset(
            k for k in (exclude_zone_keys or "").split(",") if k
        ) or None,
    )
