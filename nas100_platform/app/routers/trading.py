...
from data_source import data_source_label, get_data_source_for_user
...
def confirm_trade(body: ConfirmRequest, user: User = Depends(auth.get_current_user), db: Session = Depends(get_db)):
    ...
    own last-computed signal. There is no broker connection anywhere in
    this app, so this always just logs a paper trade for the user's own
    tracking (see each data source's place_order()) — there is no code
    path that sends a real order anywhere, or that lets one user's
    request act on another user's signal."""
